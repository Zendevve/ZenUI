-- ZenUI — targeted fix so buffs don't pop back after fade-out
...
local ADDON  = "ZenUI"
local prefix = "|cFF66C2FF[ZenUI]|r "
...
local function InitDB()
  if type(ZenUIDB)~="table" then ZenUIDB = FreshDB() end
  ZenUIDB.enabled      = asBool(ZenUIDB.enabled, true)
  ZenUIDB.debug        = asBool(ZenUIDB.debug,   false)
  ZenUIDB.showOnTarget = asBool(ZenUIDB.showOnTarget, true)
  ZenUIDB.fadeTime     = tonumber(ZenUIDB.fadeTime or 3.0)
end
local function G(n) return _G[n] end
local function dprint(msg) if ZenUIDB and ZenUIDB.debug then DEFAULT_CHAT_FRAME:AddMessage(prefix.."|cFFBBBBBB"..msg.."|r") end end
...
local FRAME_NAMES = {
  ...
  -- XP/Rep bars
  "MainMenuExpBar", "MainMenuBarMaxLevelBar", "ReputationWatchBar", "MainMenuBarArtFrame",
...
c.deferOutReason = nil
  ...
  if IsBuffLike(name) and targetAlpha == 0 and self.Fade.active and self.Fade.target == 1 then
    ...
    -- default duration
    self.Fade.duration = ClampDuration(ZenUIDB.fadeTime or 3.0)
  ...
  c.fadeCtrl:SetScript("OnUpdate", function(self, elapsed)
...
zoneHideGuard:SetScript("OnUpdate", function(self, elapsed)
  local dt = elapsed or 0
...
zoneShowGuard:SetScript("OnUpdate", function(self, elapsed)
  local dt = elapsed or 0
...
restoreFailsafe:SetScript("OnUpdate", function(self, elapsed)
  restoreFailsafe.t = restoreFailsafe.t + (elapsed or 0)
...
scheduler:SetScript("OnUpdate", function(self, elapsed)
  scheduler.timeLeft = scheduler.timeLeft - (elapsed or 0)
...
local function IsActionBarish(name)
  ...
  if name=="ZenUI_PlayerHoverFrame" or name=="PlayerFrame" or (s_match and s_match(name, "^PlayerFrame")) then return true end
...
local ZenUI_PlayerHoverFrame
local function ZenUI_SetupPlayerHover()
  if ZenUI_PlayerHoverFrame or not PlayerFrame then return end
  ZenUI_PlayerHoverFrame = CreateFrame("Frame", "ZenUI_PlayerHoverFrame", UIParent)
  ZenUI_PlayerHoverFrame:SetFrameStrata("LOW")
  ZenUI_PlayerHoverFrame:SetAllPoints(PlayerFrame)
  ZenUI_PlayerHoverFrame:EnableMouse(true)
  ZenUI_PlayerHoverFrame:Show()
end

local zenUIPlayerHoverInit = CreateFrame("Frame")
zenUIPlayerHoverInit:RegisterEvent("PLAYER_ENTERING_WORLD")
zenUIPlayerHoverInit:SetScript("OnEvent", function()
  ZenUI_SetupPlayerHover()
end)
...
function f:Evaluate(reason)
  if not ZenUIDB or not ZenUIDB.enabled then dprint("Disabled; no action."); return end
 ...
 if event == "PLAYER_ENTERING_WORLD" then
    ...
    C_TimerAfter = function(sec, fn)
        ...
        t:SetScript("OnUpdate", function(self, elapsed)
            t.elapsed = t.elapsed + (elapsed or 0)
...
dfuiPlayerHealthHider:SetScript("OnEvent", function(self, event, ...)
  if event == "ADDON_LOADED" then
    local addonName = ...
    if addonName == "-DragonflightReloaded" then
      dfuiLoaded = true
